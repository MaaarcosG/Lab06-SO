...
int main(void){
	...
	FILE *fichero;
	fichero = fopen("BitacoraMonitores.txt", "wt");
...
void *recurso(void* data){
	...
	for(int k=0; k<amount_ite; k++){
		...
		fprintf(((struct args*)data)->info, "Se consumiran %d recursos\n", count);
		...
		fprintf(((struct args*)data)->info, "Iniciando deacrease\n");
		fprintf(((struct args*)data)->info, "Mutex adquirido, entrando al monitor\n");
